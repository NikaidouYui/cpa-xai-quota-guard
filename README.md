...
5. **403 真权限** → **软禁用**（保留凭证，`permission_denied`，不自动恢复）；可用 UI「**测试/删除403**」再探测，仍 403 才 DELETE
...
| 403 chat endpoint / 真权限 | **软禁用**（不删） | **软禁用**；手动「测试/删除403」再确认可删 |
...
| `state_path` | `data/cpa-xai-quota-guard-state.sqlite` | SQLite 持久化；旧 `.json` 路径会自动迁移并保留 `.migrated.bak` |
...
| `patrol_batch_size` | `0` | 全量：0=不限，>0 截前 N；**增量巡检**：每批大小，0=默认 100 |
...
| `POST .../patrol/cpa-disabled` | 手动复查 CPA 禁用账号；200 恢复，429/402 接管追踪，失效保持禁用 |
| `POST .../patrol/permission-denied` | **测试/删除403**：仅 `permission_denied` 禁用号；200 恢复，仍 403/401 删除 |
| `POST .../patrol/incremental` | **增量巡检**：启用号按游标分批探测，完成后推进游标 |
...
- **全量**：只扫当前启用的 xAI
- **CPA 禁用复查**：仅手动触发；200 恢复，429/402 保持禁用并纳入追踪，401/403 不接管、不删除
