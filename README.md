...
CLIProxyAPI **原生 Go 插件**（当前版本 **0.3.11**）：仅针对 **xAI** 登录凭证做额度/死号管控、主动巡查、管理 UI 与用量统计。

| | |
|--|--|
| 仓库 | https://github.com/NikaidouYui/cpa-xai-quota-guard |
| 最新 Release | https://github.com/NikaidouYui/cpa-xai-quota-guard/releases/tag/v0.3.11 |
...
核心要求：下载 `v0.3.11` 对应架构 zip → 放入 `plugins/<goos>/<goarch>/` → 合并配置 → 重启 CPA → 校验 `state.version == 0.3.11`。

### Linux amd64 最短命令

```bash
VER=v0.3.11 ZIP=cpa-xai-quota-guard_0.3.11_linux_amd64.zip
...
```text
https://ghproxy.com/https://github.com/NikaidouYui/cpa-xai-quota-guard/releases/download/v0.3.11/...
```
...
Release 资产示例：`cpa-xai-quota-guard_0.3.11_linux_amd64.zip`（见 [v0.3.11](https://github.com/NikaidouYui/cpa-xai-quota-guard/releases/tag/v0.3.11)）。  
...
期望：`"version":"0.3.11"` 且插件已启用。日志：`plugin registered ... version=0.3.11`。
